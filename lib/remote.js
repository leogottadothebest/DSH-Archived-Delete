...
export class ArchivedSessionsRemote extends TypertRemoteService {
  ...
  /**
   * Unarchive every archived session in one serialized registry write: the
   * archive set is replaced once, so the workspace feed emits a single
   * `archived` increment and the sidebar restores every conversation
   * together.
   */
  async unarchiveAll(signal) {
    signal?.throwIfAborted();
    const registry = this.ctx.workspaceRegistry;
    const ids = [...registry.archivedSessionIds];
    if (ids.length === 0) return { archivedSessionIds: [] };
    await registry.enqueueOperation(async () => {
      const state = registry.requireState();
      await registry.setState({
        ...state,
  ...
  /**
   * Permanently delete every archived session, sequentially, reusing the
   * single-session deletion order (flush → detach → artifact removal →
   * registry cleanup). A per-session business failure is skipped and counted
   * in the result instead of aborting the sweep.
   */
    ...
    for (const sessionId of ids) {
      ...
      try {
        ...
        if (error instanceof TypertRemoteFailure) {
          this.ctx.logger.warn(`archived-conversations: deleteAll skipped "${sessionId}": ${error.failure.message}`);
          continue;
        }
        throw error;
      }
